...
class UserfuncComponent extends Component {
	...
	public function getLocationOptions($prearray = null, $postarray = null) {
		...
		$items = ClassRegistry::init ( $modelName )->find ( 'list', array (
	...
	public function getLocationCondition($model = 'User', $condition = null) {
		...
		if ($this->Session->read ( 'user_role' ) !== 'admin') {
			$t = array (
					$model . '.location_id' => $this->Session->read ( 'user_location' ) 
			);
	...
	public function can_modify($rec_owner, $user_info) {
		$result = false;
		if (($this->is_book_admin($user_info)) || ($user_info ["id"] == $rec_owner)) {
			 $result = true;
		}
		return $result;
	}
	public function is_book_admin($user_info) {
		$result = false;
		$db = ClassRegistry::init ( "User" )->getDataSource ();
